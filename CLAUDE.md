...
2. **`download_images.sh`**: Main workflow script that orchestrates the entire image processing pipeline
3. **`cleanup.sh`**: Utility script for removing generated files and temporary directories

### Image Processing Pipeline

The `download_images.sh` script implements am image processing workflow:
